...
data Context = context(Environment env, Store sto, Display dis, Flow flow, list[FormElement] res);
...
Context do_visit(Id name, Context c) {
  if (normalized_question(_, name, _, Expr cond, Maybe[Expr] def) := c.env[name]) {  
    def_old = c.sto[name];
    cond_old = c.dis[name];
    c = assignments(name, cond, def, c);
    if (def_old != c.sto[name] || cond_old != c.dis[name]) {
      c = output_form_element(name, c);
    }
...
Context assignments(Id name, Expr cond, Maybe[Expr] def, Context c) {
...
Context output_form_element(name, c) {
